	...
	if (sequence_of_vertices.size() < points.size() - 1) { // if the next vertex will not be the last
		for (int next_vertex = 1; next_vertex < points.size(); ++next_vertex) { // trying to add new vertex
			if (find(sequence_of_vertices.begin(), sequence_of_vertices.end(), next_vertex) == sequence_of_vertices.end()) { //adding unique new vertex
				T new_length = currently_length;
				new_length += sqrt(pow(points[next_vertex].first - points[*(--sequence_of_vertices.end())].first, 2) +
					                     pow(points[next_vertex].second - points[*(--sequence_of_vertices.end())].second, 2));
				sequence_of_vertices.push_back(next_vertex);
				if (new_length < min_cycle) { // in this case we trying to continue our way
					DFS(points, sequence_of_vertices, new_length, min_cycle);
				}
		...
		for (int next_vertex = 1; next_vertex < points.size(); ++next_vertex) { // trying to add new vertex
			if (find(sequence_of_vertices.begin(), sequence_of_vertices.end(), next_vertex) == sequence_of_vertices.end()) { //adding unique new vertex
				...
				new_length += sqrt(pow(points[0].first - points[*(--sequence_of_vertices.end())].first, 2) +
					                     pow(points[0].second - points[*(--sequence_of_vertices.end())].second, 2));
				if (new_length < min_cycle) { // if the new cycle is better
					// we update the best (min) cycle
					min_cycle = new_length;
				}
				sequence_of_vertices.pop_back();
			}
		}
