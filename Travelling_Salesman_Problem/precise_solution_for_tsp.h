	...
	if (sequence_of_vertices.size() < points.size() - 1) { // if the next vertex will not be the last
		for (int next_vertex = 1; next_vertex < points.size(); ++next_vertex) { // trying to add new vertex
			if (find(sequence_of_vertices.begin(), sequence_of_vertices.end(), next_vertex) == sequence_of_vertices.end()) { //adding unique new vertex
				currently_length += sqrt(pow(points[next_vertex].first - points[sequence_of_vertices.size() - 1].first, 2) +
					                     pow(points[next_vertex].second - points[sequence_of_vertices.size() - 1].second, 2));
		...
		for (int next_vertex = 1; next_vertex < points.size(); ++next_vertex) { // trying to add new vertex
			if (find(sequence_of_vertices.begin(), sequence_of_vertices.end(), next_vertex) == sequence_of_vertices.end()) { //adding unique new vertex
				...
				currently_length += sqrt(pow(points[0].first - points[sequence_of_vertices.size() - 1].first, 2) + pow(points[0].second - points[sequence_of_vertices.size() - 1].second, 2));
